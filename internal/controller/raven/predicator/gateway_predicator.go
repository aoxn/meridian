...
func (e *EnqueueGatewayForNode) Create(ctx context.Context, evt event.CreateEvent, q workqueue.RateLimitingInterface) {
	...
	if !ok {
		klog.Error("could not assert runtime Object to v1.Node")
		return
	}
	klog.V(5).Infof("will enqueue gateway as node(%s) has been created",
		node.GetName())
	if gwName, exist := node.Labels[raven.LabelCurrentGateway]; exist {
		util.AddGatewayToWorkQueue(gwName, q)
		return
	}
	klog.V(4).Infof("node(%s) does not belong to any gateway", node.GetName())
}
...
func (e *EnqueueGatewayForNode) Update(ctx context.Context, evt event.UpdateEvent, q workqueue.RateLimitingInterface) {
	...
	if !ok {
		klog.Errorf("could not assert runtime Object(%s) to v1.Node",
	...
	klog.V(5).Infof("Will enqueue gateway as node(%s) has been updated",
		newNode.GetName())
...
func (e *EnqueueGatewayForNode) Delete(ctx context.Context, evt event.DeleteEvent, q workqueue.RateLimitingInterface) {
	...
	if !exist {
		klog.V(5).Infof("Node(%s) doesn't belong to any gateway", node.GetName())
		return
	}
...
func NewRavenGateway(client client.Client) *EnqueueGatewayForRavenConfig {
	return &EnqueueGatewayForRavenConfig{client: client}
}
...
func (e *EnqueueGatewayForRavenConfig) Create(ctx context.Context, evt event.CreateEvent, q workqueue.RateLimitingInterface) {
	...
	if !ok {
		klog.Error("could not assert runtime Object to v1.ConfigMap")
		return
	}
	klog.V(2).Infof("Will config all gateway as raven-cfg has been created")
	if err := e.enqueueGateways(q); err != nil {
		klog.Errorf("could not config all gateway, error %s", err.Error())
		return
	}
}
...
func (e *EnqueueGatewayForRavenConfig) Update(ctx context.Context, evt event.UpdateEvent, q workqueue.RateLimitingInterface) {
	...
	if oldCm.Data[util.RavenEnableProxy] != newCm.Data[util.RavenEnableProxy] {
		klog.V(4).Infof("Will config all gateway as raven-cfg has been updated")
		if err := e.enqueueGateways(q); err != nil {
			klog.Errorf("could not config all gateway, error %s", err.Error())
			return
		}
...
func (e *EnqueueGatewayForRavenConfig) Delete(ctx context.Context, evt event.DeleteEvent, q workqueue.RateLimitingInterface) {
	...
	klog.V(4).Infof("Will config all gateway as raven-cfg has been deleted")
	if err := e.enqueueGateways(q); err != nil {
		klog.Errorf("could not config all gateway, error %s", err.Error())
		return
	}
