...
func AddGatewayPublicService(
	...
	return addGatewayPubService(mgr, newGatewayPubServiceReconciler(mgr))
}

// newReconciler returns a new reconcile.Reconciler
func newGatewayPubServiceReconciler(
	mgr manager.Manager,
) reconcile.Reconciler {
	recon := &reconcileGatewayPublicService{
		Client:   mgr.GetClient(),
		scheme:   mgr.GetScheme(),
		recorder: mgr.GetEventRecorderFor("GatewayPublicService"),
	}
	return recon
}

// add adds a new Controller to mgr with r as the reconcile.Reconciler
func addGatewayPubService(mgr manager.Manager, r reconcile.Reconciler) error {
	// Create a new controller
	c, err := controller.New(
		"gateway-pubservice-controller", mgr,
		controller.Options{
			Reconciler:              r,
	...
	err = c.Watch(
		...
		&predicator.EnqueueRequestForGatewayEvent{},
	)
	if err != nil {
		return err
	}
	...
	return c.Watch(
		...
		predicator.NewRequestForConfigEvent(mgr.GetClient()),
...
var _ reconcile.Reconciler = &reconcileGatewayPublicService{}
...
// reconcileGatewayPublicService reconciles a Gateway object
type reconcileGatewayPublicService struct {
	client.Client
...
func (r *reconcileGatewayPublicService) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		if apierrs.IsNotFound(err) {
			...
			klog.Errorf("could not get gateway %s, error %s", req.Name, err.Error())
...
func (r *reconcileGatewayPublicService) getGateway(ctx context.Context, req reconcile.Request) (*ravenv1beta1.Gateway, error) {
...
func (r *reconcileGatewayPublicService) reconcileService(ctx context.Context, gw *ravenv1beta1.Gateway, record *serviceRecord, enableTunnel, enableProxy bool) error {
	if enableProxy {
...
func (r *reconcileGatewayPublicService) reconcileEndpoints(ctx context.Context, gw *ravenv1beta1.Gateway, record *serviceRecord, enableTunnel, enableProxy bool) error {
	if enableProxy {
...
func (r *reconcileGatewayPublicService) clearService(ctx context.Context, gatewayName, gatewayType string) error {
...
func (r *reconcileGatewayPublicService) clearEndpoints(ctx context.Context, gatewayName, gatewayType string) error {
...
func (r *reconcileGatewayPublicService) manageService(ctx context.Context, gateway *ravenv1beta1.Gateway, gatewayType string, record *serviceRecord) error {
	...
	for i := 0; i < len(addSvc); i++ {
		if err := r.Create(ctx, addSvc[i]); err != nil {
			if apierrs.IsAlreadyExists(err) {
				klog.V(2).Infof("service %s/%s has already exist, ignore creating it", addSvc[i].GetNamespace(), addSvc[i].GetName())
				return nil
			}
...
func (r *reconcileGatewayPublicService) manageEndpoints(ctx context.Context, gateway *ravenv1beta1.Gateway, gatewayType string, record *serviceRecord) error {
	...
	for i := 0; i < len(addEps); i++ {
		if err := r.Create(ctx, addEps[i]); err != nil {
			if apierrs.IsAlreadyExists(err) {
				klog.V(2).Infof("endpoints %s/%s has already exist, ignore creating it", addEps[i].GetNamespace(), addEps[i].GetName())
				return nil
			}
...
func (r *reconcileGatewayPublicService) getTargetPort() (proxyPort, tunnelPort int32) {
...
func (r *reconcileGatewayPublicService) listService(ctx context.Context, gatewayName, gatewayType string) (*corev1.ServiceList, error) {
...
func (r *reconcileGatewayPublicService) listEndpoints(ctx context.Context, gatewayName, gatewayType string) (*corev1.EndpointsList, error) {
...
func (r *reconcileGatewayPublicService) acquiredSpecEndpoints(ctx context.Context, gateway *ravenv1beta1.Gateway, gatewayType string, record *serviceRecord) *corev1.EndpointsList {
	proxyPort, tunnelPort := r.getTargetPort()
...
func (r *reconcileGatewayPublicService) getEndpointsAddress(ctx context.Context, name string) (*corev1.EndpointAddress, error) {
	var node corev1.Node
	err := r.Get(ctx, types.NamespacedName{Name: name}, &node)
	if err != nil {
		klog.Errorf("could not get node %s for get active endpoints address, error %s", name, err.Error())
		return nil, err
	}
