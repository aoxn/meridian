...
import (
	...
	nativeimgutil "github.com/aoxn/meridian/internal/vma/nativeimg"
...
func createInitialConfig(driver *backend.BaseDriver) (*vz.VirtualMachineConfiguration, error) {
	...
	bytes, err := units.RAMInBytes(driver.Yaml.Spec.Memory)
	if err != nil {
		return nil, err
	}
...
func attachDisks(driver *backend.BaseDriver, vmConfig *vz.VirtualMachineConfiguration) error {
	...
	for _, d := range driver.Yaml.Spec.AdditionalDisks {
		diskName := d.Name
...
func attachDisplay(driver *backend.BaseDriver, vmConfig *vz.VirtualMachineConfiguration) error {
	switch driver.Yaml.Spec.Video.Display {
	case "vz", "default":
	...
	case "none":
		return nil
	default:
		return fmt.Errorf("unexpected video display %q", driver.Yaml.Spec.Video.Display)
	}
}
...
func attachFolderMounts(driver *backend.BaseDriver, vmConfig *vz.VirtualMachineConfiguration) error {
	...
	for i, mount := range driver.Yaml.Spec.Mounts {
...
func attachAudio(driver *backend.BaseDriver, config *vz.VirtualMachineConfiguration) error {
	switch driver.Yaml.Spec.Audio.Device {
	case "vz", "default":
	...
	case "", "none":
		return nil
	default:
		return fmt.Errorf("unexpected audio device %q", driver.Yaml.Spec.Audio.Device)
	}
}
