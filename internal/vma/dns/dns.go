...
func newStaticClientConfig(ips []string) (*dns.ClientConfig, error) {
	klog.Infof("newStaticClientConfig creating config for the following IPs: %v", ips)
	s := ``
	for _, ip := range ips {
...
func NewHandler(opts HandlerOptions) (dns.Handler, error) {
	...
	if len(opts.UpstreamServers) == 0 {
		if runtime.GOOS != "windows" {
			...
			if err != nil {
				klog.Warningf("failed to detect system XdpDomain, falling back to %v, %s", defaultFallbackIPs, err.Error())
		...
		if cc, err = newStaticClientConfig(opts.UpstreamServers); err != nil {
			klog.Warningf("failed to create a client config from: %v, falling back to %v, %s", opts.UpstreamServers, defaultFallbackIPs, err.Error())
...
func (h *Handler) handleQuery(w dns.ResponseWriter, req *dns.Msg) {
	...
	defer w.Close()
	reply.SetReply(req)
	klog.Infof("handleQuery received XdpDomain query: %v", req)
	for _, q := range req.Question {
		...
		case dns.TypeA:
			...
			if _, ok := h.hostToIP[cname]; ok {
				...
				if err != nil {
					klog.Errorf("handleQuery lookup IP failed: %s", err.Error())
					continue
				}
			}
		...
		case dns.TypeCNAME:
			...
			if _, ok := h.hostToIP[cname]; !ok {
				...
				if err != nil {
					klog.Errorf("handleQuery lookup CNAME failed: %s", err.Error())
					continue
				}
			}
		...
		case dns.TypeTXT:
			...
			if err != nil {
				klog.Errorf("handleQuery lookup TXT failed: %s", err.Error())
				continue
			}
			for _, s := range txt {
				a := &dns.TXT{
		...
		case dns.TypeNS:
			...
			if err != nil {
				klog.Errorf("handleQuery lookup NS failed: %s", err.Error())
				continue
			}
			for _, s := range ns {
				if s.Host != "" {
		...
		case dns.TypeMX:
			...
			if err != nil {
				klog.Errorf("handleQuery lookup MX failed: %s", err.Error())
				continue
			}
			for _, s := range mx {
				if s.Host != "" {
		...
		case dns.TypeSRV:
			...
			if err != nil {
				klog.Errorf("handleQuery lookup SRV failed: %s", err.Error())
				continue
			}
	...
	if handled {
		...
		if err := w.WriteMsg(&reply); err != nil {
			klog.Errorf("handleQuery failed writing XdpDomain reply: %s", err.Error())
		}
...
func (h *Handler) handleDefault(w dns.ResponseWriter, req *dns.Msg) {
	klog.Infof("handleDefault for %v", req)
	for _, client := range h.clients {
		for _, srv := range h.clientConfig.Servers {
			addr := net.JoinHostPort(srv, h.clientConfig.Port)
			reply, _, err := client.Exchange(req, addr)
			if err != nil {
				klog.Errorf("handleDefault failed to perform a synchronous query with upstream [%v], %s", addr, err.Error())
				continue
			}
			if h.truncate {
				klog.Infof("handleDefault truncating reply: %v", reply)
				reply.Truncate(truncateSize)
			}
			if err = w.WriteMsg(reply); err != nil {
				klog.Errorf("handleDefault failed writing XdpDomain reply to [%v]: %s", addr, err.Error())
			}
			return
		}
	}
	...
	if err := w.WriteMsg(&reply); err != nil {
		klog.Errorf("handleDefault failed writing XdpDomain reply: %s", err.Error())
	}
}
...
func listenAndServe(network Network, opts ServerOptions) (*dns.Server, error) {
	...
	go func() {
		klog.Infof("Start %v XdpDomain listening on: %v", network, addr)
