...
func NewConnectivity(fwd *forward.ForwardMgr, driver backend.Driver, vm *model.Instance) *Connectivity {
	return &Connectivity{
		fwd: fwd, driver: driver, vm: vm,
	}
}
...
func (ha *Connectivity) Forward(ctx context.Context) error {
	...
	forwards := ha.vm.Spec.PortForwards
...
func (ha *Connectivity) SetMappingRoute() {
	...
	if !v1.HasFeature(
		ha.vm.Vm().Request.Config.Features, v1.FeatureSupportNodeGroups) {
