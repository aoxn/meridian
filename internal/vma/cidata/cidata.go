...
func NewCloudInit(i *model.Instance, sshmgr *sshutil.SSHMgr) *CloudInit {
...
func (i *CloudInit) GenCIISO() error {
	...
	args := TemplateArgs{
		Name:       i.ii.Name,
		User:       u,
		VMType:     string(i.ii.Spec.VMType),
		TimeZone:   i.ii.Spec.TimeZone,
		SSHPubKeys: pubs,
		MountType:  "virtiofs",
		CACerts: CACerts{
	...
	for k, n := range i.ii.Spec.Mounts {
		mount := Mount{
			MountPoint: n.MountPoint,
			Type:       "virtiofs",
			Tag:        fmt.Sprintf("mount%d", k),
		}
		if i.ii.Spec.VMType == "vz" {
			mount.Type = "virtiofs"
		}
		args.Mounts = append(args.Mounts, mount)
	}

	for _, n := range i.ii.Spec.Networks {
		network := Network{
			Interface:  "enp0s1",
