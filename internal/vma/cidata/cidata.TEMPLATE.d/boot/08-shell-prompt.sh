...
if [ "${MD_CIDATA_NAME}" = "default" ] && command -v patch >/dev/null 2>&1 && grep -q color_prompt "${MD_CIDATA_HOME}/.bashrc"; then

	! grep -q "^# Meridian PS1" "${MD_CIDATA_HOME}/.bashrc" || exit 0

	# Change the default shell prompt from "green" to "lime green" (#32CD32)

	patch --forward -r - "${MD_CIDATA_HOME}/.bashrc" <<'EOF'
@@ -37,7 +37,11 @@
