...
if command -v iptables >/dev/null 2>&1; then
	...
	if [ -n "${MD_CIDATA_UDP_DNS_LOCAL_PORT}" ] && [ "${MD_CIDATA_UDP_DNS_LOCAL_PORT}" -ne 0 ]; then
		iptables --table nat --append "${chain}" --destination "${MD_CIDATA_SLIRP_DNS}" --protocol udp --dport 53 --jump DNAT \
			--to-destination "${MD_CIDATA_SLIRP_GATEWAY}:${MD_CIDATA_UDP_DNS_LOCAL_PORT}"
	fi
	if [ -n "${MD_CIDATA_TCP_DNS_LOCAL_PORT}" ] && [ "${MD_CIDATA_TCP_DNS_LOCAL_PORT}" -ne 0 ]; then
		iptables --table nat --append "${chain}" --destination "${MD_CIDATA_SLIRP_DNS}" --protocol tcp --dport 53 --jump DNAT \
			--to-destination "${MD_CIDATA_SLIRP_GATEWAY}:${MD_CIDATA_TCP_DNS_LOCAL_PORT}"
	fi
fi
