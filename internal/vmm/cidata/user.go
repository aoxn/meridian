...
func MdUser(warn bool) (*user.User, error) {
	cache.warnings = []string{}
	cache.Do(func() {
		u := os.Getenv("SUDO_USER")
		if u != "" {
			cache.u, cache.err = user.Lookup(u)
		} else {
		...
		if cache.err == nil {
			...
			if runtime.GOOS == "windows" {
				...
				if err != nil {
					klog.Errorf("call cygpath %s", err)
				}
				if home == "" {
	...
	if warn && len(cache.warnings) > 0 {
		for _, warning := range cache.warnings {
			klog.Warningf("md user warning: %s", warning)
		}
	}
