...
func (ssh *SSHMgr) SSHConfig(instDir string) (*sshc.SSHConfig, error) {
	...
	sshOpts, err := ssh.SSHOpts(instDir, true, true, true, true)
	if err != nil {
		return nil, err
	}
...
func (ssh *SSHMgr) SSHOpts(instDir string, useDotSSH, forwardAgent, forwardX11, forwardX11Trusted bool) ([]string, error) {
	controlSock := filepath.Join(instDir, v1.SSHSock)
	if len(controlSock) >= 256 {
		return nil, fmt.Errorf("socket path %q is too long: >= UNIX_PATH_MAX=%d", controlSock, 256)
	}
	u, err := MdUser(false)
	if err != nil {
		return nil, err
	}
	opts, err := ssh.CommonOpts(useDotSSH)
	if err != nil {
		return nil, err
	}
