...
func ConvertToRaw(source, dest string, size *int64, allowSourceWithBackingFile bool) error {
	...
	klog.Infof("Converting %q (%s) to a raw disk %q", source, srcImg.Type(), dest)
	...
	default:
		klog.Warningf("image %q has an unexpected format: %q", source, t)
	}
	...
	destTmpF, err := os.CreateTemp(filepath.Dir(dest), filepath.Base(dest)+".md-*.tmp")
	if err != nil {
		return err
	}
	...
	// Resize
	if size != nil {
		klog.Infof("Expanding to %s", units.BytesSize(float64(*size)))
...
func MakeSparse(f *os.File, n int64) error {
	...
	return unix.Ftruncate(int(f.Fd()), n)
}
